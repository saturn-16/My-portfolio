...
function Band({
  ...
  // Rope joint limit: increased from original 1 to 2 for a longer strap
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current) {
      ...
      // Offset strap endpoint Z to match clip visual position (card group z=0.1)
      const j3Pos = j3.current.translation();
      curve.points[0].set(j3Pos.x, j3Pos.y, j3Pos.z + 0.1);
  ...
  return (
    <>
      {/* --- ORIGINAL LAYOUT: untouched from React Bits source --- */}
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          <group
            ...
            onPointerUp={(e: any) => {
              ...
              // Detect click vs drag: if pointer barely moved, it's a click
              const dx = e.clientX - (e.target as any).__downX;
              const dy = e.clientY - (e.target as any).__downY;
            ...
            onPointerDown={(e: any) => {
              ...
              (e.target as any).__downX = e.clientX;
              (e.target as any).__downY = e.clientY;
      ...
      {/* --- ORIGINAL STRAP: untouched from React Bits source --- */}
      <mesh ref={band}>
        <meshLineGeometry />
        <meshLineMaterial
          color="white"
